...
class HuntDestroyAiPlayerTest {
  ...
  static HuntDestroyAiPlayer testAi;
  Coord coord1;
  Coord coord2;
  Coord coord3;
  Coord coord4;
  ...
  /**
   * Initializes ship specs to test LocalRandomAiPlayer
   */
  @BeforeAll
  public static void setup() {
    ...
    testAi = new HuntDestroyAiPlayer(PLAYER_NAME, new PlayerBoards(SEED), SEED);
    testAi.setup(BOARD_HEIGHT, BOARD_WIDTH, specs);
  }

  /**
   * Tests HuntDestroyAiPlayer's constructor and takeShots()
   */
  @Test
  public void testConstructorAndShots() {
    ...
    coord2 = new Coord(3, 0);
    coord3 = new Coord(5, 2);
    coord4 = new Coord(0, 3);
    coord5 = new Coord(5, 5);
    coord6 = new Coord(0, 6);
    ...
    testAi.successfulHits(new ArrayList<>(List.of(coord1)));
    testAiShots = (ArrayList<Coord>) testAi.takeShots();
    assertEquals(5, testAiShots.size());

    // Cross-exploration around (0, 0)
    coord1 = new Coord(1, 0);
    coord2 = new Coord(0, 1);
    // Back to hunt - outer ring
    coord3 = new Coord(3, 6);
    // Hunt - next ring
    coord4 = new Coord(1, 1);
    coord5 = new Coord(4, 1);
